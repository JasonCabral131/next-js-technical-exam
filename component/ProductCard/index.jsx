  ...
  return (
    ...
    <div className="w-100">
      ...
      <Link href={`/${product?.id}`} className="pointer">
       <div className='pointer current-search-name'>{product?.fields.Name}</div> 
        </Link>
      </div>
      ...
      <MdOutlineCategory/> <div style={{marginLeft: '5px'}}>  {product?.fields.Brand}</div>
      </div>
      <div className="current-search-description">
        " {product?.fields.Description} "
      </div>
      {
        !isview ?     <Link href={`/${product?.id}`} className="pointer ">
